...
st.write("Inspiration for this project has come from me not wanting to have to rely on tools like Quizlet and NotebookLM to study my notes since I have ADHD and I strongly detest studying and I don't enjoy school outside of computers. This tool is meant to help me and others that want this material to learn material to a depth that hasn't been done before and also improve the speed of learning.")
st.markdown("## What it does")
st.write("This AI tool uses imported texts [supports MD files, PDFs, etc] and also image files to extract information to aid studying like a professional RAG would do.")
st.markdown("## How I built it")
st.write("I used Python as the framework, utilizing both the CLI aspect and the Web UI application using Streamlit. These both have different features that are available, only the Web UI can upload photos directly from the camera but that may change in later updates if people desire a change.")
...
st.write("I plan to be adding more and more onto Study Assistant so if you want later versions go to this project's [GitHub repo](https://github.com/pymite6941/study-assistant) for the updates and for the whole process.")